...
export function ActivityTimeline({
  ...
  const getEventIcon = (title: string, index: number) => {
    ...
    if (title.toLowerCase().includes("generating") || title.includes("generate")) {
      return <TextSearch className="h-4 w-4 text-neutral-400" />;
    } else if (title.toLowerCase().includes("thinking") || title.includes("think")) {
      return <Loader2 className="h-4 w-4 text-neutral-400 animate-spin" />;
    } else if (title.toLowerCase().includes("reflection") || title.includes("reflect")) {
      return <Brain className="h-4 w-4 text-neutral-400" />;
    } else if (title.toLowerCase().includes("research") || title.includes("research")) {
      return <Search className="h-4 w-4 text-neutral-400" />;
    } else if (title.toLowerCase().includes("finalizing") || title.includes("final")) {
